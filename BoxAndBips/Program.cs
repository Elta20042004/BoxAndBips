...
namespace Game
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            World w = new World(14, 15,new[] {new DominanceRule()});
            w.PutSpeedBip(1, 2, 0);
            w.PutSpeedBip(3, 2, 0);
            w.PutSpeedBip(0, 1, 0);
            w.PutLifeBip(1, 2, 5);
            ...
            var box = new Box("box1", 20, w);
            box.X = 0;
            box.Y = 0;
            w.PutBox(box, 0, 0);

            Box robotBoxI = new Box("robot-I", 15, w);
            robotBoxI.X = 3;
            robotBoxI.Y = 3;
            w.PutBox(robotBoxI,3,3);

            Box robotBoxII = new Box("robot-II", 15, w);
            robotBoxII.X = 4;
            robotBoxII.Y = 4;
            w.PutBox(robotBoxII, 4, 4);

            Box robotBoxIII = new Box("robot-III", 15, w);
            robotBoxIII.X = 10;
            robotBoxIII.Y = 10;
